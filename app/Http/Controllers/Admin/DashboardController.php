...
class DashboardController extends Controller
{
    public function index($other_user_id = null)
    {
        ...
        //New Session Generate for lead Type
        $team_info = TeamAssign::where("user_pk_no", $user_id)->first();
        session(['user.ses_lead_type' => isset($team_info->lead_type) ? $team_info->lead_type : 0]);
        ...
        $lead_count = DB::select("SELECT COUNT(1) total_lead
			...
			WHERE lead_current_stage in(1,10,11) $user_cond");
        ...
        $upcomming = DB::select("SELECT COUNT(1) total_lead
			FROM t_leadlifecycle llc
			WHERE lead_current_stage=8
			 $user_cond");

        $followup = DB::select("SELECT COUNT(1) total_lead
			FROM t_leadlifecycle llc
			WHERE lead_current_stage=10
			 $user_cond");

        $call_again = DB::select("SELECT COUNT(1) total_lead
			FROM t_leadlifecycle llc
			WHERE lead_current_stage=11
			 $user_cond");

        $unadrressed = DB::select("SELECT COUNT(1) total_lead
			FROM t_leadlifecycle llc
        ...
        return view('admin.dashboard', compact('lead_count', 'k1', 'priority', 'sold', 'hold', 'closed', 'transferred', 'accepted', 'avt_data', 'apt_data', 'acr_data', 'next_followup', 'missed_followup', 'digital_mkt', 'hotline', "upcomming", "followup", "call_again", "unadrressed"));
    }
