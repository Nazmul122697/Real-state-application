            ...
            <div class="row">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select class="form-control select2 required" id="cmbTransferTo" name="cmbTransferTo"
                            style="width: 100%;" aria-hidden="true">
                            <option selected="selected" value="">Select Sales Agent</option>
                            @foreach ($sales_agent as $key => $agent)
                                <option Value="{{ $agent->user_pk_no }}"
                                    data-agent-category="{{ $agent->lookup_pk_no }}">
                ...
                <tr>
                    ...
                    <th class="text-center">
                        ...
                        <a href="#" class="btn bg-blue btn-block btn-xs btn-transfer"
                            data-response-action="{{ route('load_transfer_leads') }}">Transfer</a>
                    </th>
                ...
                @if (!empty($lead_transfer_list))
                    @foreach ($lead_transfer_list as $row)
                        <tr>
                            ...
                            <td class="text-center">{{ $row->customer_firstname . ' ' . $row->customer_lastname }}
                            </td>
                            ...
                            <td class="text-center">
                                {{ isset($followup_arr[$row->lead_pk_no]['lead_followup_datetime']) ? $followup_arr[$row->lead_pk_no]['lead_followup_datetime'] : '' }}
                            </td>
                            <td class="text-center">
                                {{ isset($followup_arr[$row->lead_pk_no]['followup_Note']) ? $followup_arr[$row->lead_pk_no]['followup_Note'] : '' }}
                            </td>
                            <td class="text-center">
                                {{ isset($followup_arr[$row->lead_pk_no]['Next_FollowUp_date']) ? $followup_arr[$row->lead_pk_no]['Next_FollowUp_date'] : '' }}
                            </td>
                            <td class="text-center">
                                <input type="checkbox" data-id="{{ $row->lead_pk_no }}"
