...
@section('content')
    ...
    $role_type = Session::get('user.user_type');
                                ...
                                @if (!empty($lead_data))
                                    ...
                                    @foreach ($lead_data as $row)
                                        <tr>
                                            ...
                                            <td class="text-">{{ $row->lead_id }}
                                                @if ($row->lead_current_stage == 7 && $row->is_cancel == 1)
                                            ...
                                            <td>{{ $row->lead_sales_agent_name == '' ? $row->user_fullname : $row->lead_sales_agent_name }}
                                            </td>
                                            ...
                                            <td class="text-center" width="100px;">
                                                ...
                                                {{-- @if ($is_super_admin) --}}
                                                    <span class="btn bg-info btn-xs lead-edit" data-title="Lead Edit"
                                                        title="Lead Edit" data-id="{{ $row->lead_pk_no }}"
                                                ...
                                                @if ($role_type == 2||$is_super_admin)
                                                    @if ($row->lead_current_stage != 7)
                                                    ...
                                                    @endif
                                                    @if ($row->lead_current_stage == 7 && $row->is_cancel != 1)
                                                        <span data-action="{{ route('cancel_sale', $row->lead_pk_no) }}"
                                                            title="Cancel Sales" class="btn btn-danger btn-xs cancel_sale">
...
@push('js_custom')
    ...
    <script>
        $(function() {
            $(document).on("click", ".cancel_sale", function(e) {
                blockUI();
                var action = $(this).attr('data-action');
                if (confirm("Are You Sure to cancel this sale?")) {
                    $.ajax({
