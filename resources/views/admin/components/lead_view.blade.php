    ...
    <div class="box-body">
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <h5>{{ isset($lead_type[$lead_data->lead_type]) ? $lead_type[$lead_data->lead_type] : '' }}</h5>
            </div>
            <div class="col-md-4">
        ...
        <div class="row">
            ...
            <div class="col-md-4">
                <label>Road No :</label>
                <h5>{{ $lead_data->road_no }}</h5>
            </div>
            <div class="col-md-4">
                <label>Block No :</label>
                <h5>{{ $lead_data->block_no }}</h5>
            </div>
            <div class="col-md-4">
                <label>Plot No :</label>
                <h5>{{ $lead_data->plot_no }}</h5>
            </div>
            <div class="col-md-4">
                <label>Sector No :</label>
                <h5>{{ $lead_data->sector_no }}</h5>
            </div>
            <div class="col-md-4">
                <label>Details :</label>
                <h5>{{ $lead_data->details }}</h5>
            </div>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    @foreach ($lead_history as $lead_hist)
                        <tr>
                            <td scope="row">{{ $lead_hist->full_name }}</td>
    ...
    <div class="box-body">
        <div class="row">
            <div class="col-md-4">
                ...
                <h5>{{ $lead_data->Customer_dateofbirth != '0000-01-01' ? date('d-m-Y', strtotime($lead_data->Customer_dateofbirth)) : '' }}
                </h5>
            </div>
            ...
            <div class="col-md-4">
                ...
                <h5>{{ $lead_data->customer_wife_dataofbirth != '0000-01-01' ? date('d-m-Y', strtotime($lead_data->customer_wife_dataofbirth)) : '' }}
                </h5>
            </div>

            <div class="col-md-4">
                <label for="">Marriage Anniversary :</label>
                <h5>{{ $lead_data->Marriage_anniversary != '0000-01-01' ? date('d-m-Y', strtotime($lead_data->Marriage_anniversary)) : '' }}
                </h5>
            </div>
            ...
            <div class="col-md-4">
                ...
                <h5>{{ $lead_data->children_dateofbirth1 != '0000-01-01' ? date('d-m-Y', strtotime($lead_data->children_dateofbirth1)) : '' }}
                </h5>
            </div>
            @if ($lead_data->lead_type == 2 || $lead_data->lead_type == 11 || $lead_data->lead_type == 7 || $lead_data->lead_type == 9)
                <div class="col-md-4">
                    <label for="">Bed Number :</label>
                    <h5>{{ $lead_data->bed_number }}</h5>
                </div>

                <div class="col-md-4">
                    <label for="">Budger Range :</label>
                    <h5>{{ $lead_data->budget_range }} </h5>
                </div>
            @endif
...
@php
$k_qualification = json_decode($lead_data->k_qualification, true);
@endphp
@if (!empty($k_qualification))
    <div class="box" style="border-color:#9384ff;">
        <div class="box-header with-border">
            <h3 class="box-title">K Qualification</h3>
        </div>
        <div class="box-body">
            <div class="row">
                <div class="col-md-12">
                    <div class="row">
                        @if (!empty($k_qualification))
                            @foreach ($k_qualification as $key => $k)

                                <div class="col-md-6">
                                    <input type="checkbox" value="{{ $k }}"
                                        {{ $k == 1 ? 'checked' : '' }} disabled>
                                    @if ($key == 'match_location')
                                        Match With Location
                                    @elseif ($key == 'ready_ongoing_flat')
                                        Ready or Ongoing flat
                                    @elseif ($key == 'square_feet')
                                        Match the size/square feet
                                    @elseif ($key == 'match_budget')
                                        Match the budget
                                    @elseif ($key == 'floor_design')
                                        Floor/Design preferance
                                    @elseif ($key == 'time_purchase')
                                        Time of Purchase
                                    @endif
                                </div>
                            @endforeach
                ...
                <tbody>
                    ...
                    @foreach ($lead_transfer_data as $trans_hist)
                        <tr>
                            ...
                            <td scope="row">{{ date('d-m-Y', strtotime($trans_hist->updated_at)) }}</td>
