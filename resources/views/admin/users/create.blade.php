    ...
    <section class="content">
        <div class="row">
            <div class="col-md-6">
                ...
                <div class="form-group">
                    <label for="cmbUserGroup">Designation
                        <small style="color:red">*</small>
                    </label>
                    <select name="cmbUserDesignation" id="cmbUserDesignation" class="form-control"
                            style="width: 100%;" required="required" aria-hidden="true">
                        <option value="0">Select</option>

                        @foreach ($designations as $designation)
                            @if (!empty($user) && $user->designation_lookup_id == $designation->lookup_pk_no)
                                <option value="{{ $designation->lookup_pk_no }}" selected>{{ $designation->lookup_name }}</option>
                            @else
                                <option value="{{ $designation->lookup_pk_no }}">{{ $designation->lookup_name }}</option>
                            @endif
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <label for="txtEmail">Employee ID
                        <small style="color:red">*</small>
                    </label>
                    <input type="text" class="form-control required" id="txtEmployeeID" name="txtEmployeeID" title="Employee ID"
                           placeholder="" value="{{ isset($user)? $user->employee_id:'' }}"
                ...
                <div class="form-group">
                    <label for="txtEmail">Joining Date
                        <small style="color:red">*</small>
                    </label>
                    <input type="text" class="form-control required datepicker" readonly id="txtJoiningDate" name="txtJoiningDate"
                           title="" placeholder="" value="{{isset($user)? ($user->joining_date!=""?date("d-m-Y", strtotime($user->joining_date)):""):''}}" required="required" tabindex="2">
                </div>

                <div class="form-group">
                    <label for="txtEmail">Reporting Date
                        <small style="color:red">*</small>
                    </label>
                    <input type="text" class="form-control required datepicker" readonly id="txtReportingDate" name="txtReportingDate"
                           title="" placeholder="" value="{{isset($user)? ($user->reporting_date!=""?date("d-m-Y", strtotime($user->reporting_date)):""):''}}" required="required" tabindex="2">
                ...
                <div class="form-group">
                    ...
                    <select name="cmbUserStatus" id="cmbUserStatus" class="form-control"
                        ...
                        <option value="1" {{ !empty($user)?($user->status == 1)? 'selected="selected"':'':'' }}>Active</option>
                        <option value="0" {{ !empty($user)?($user->status == 0)? 'selected="selected"':'':'' }}>Inactive</option>
                    </select>
                </div>
            </div>
        </div>
