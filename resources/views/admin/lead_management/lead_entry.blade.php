...
@endphp
{{-- @dd($user_type) --}}
@if($user_type==2&&($is_ses_hod||$is_ses_hot||$is_team_leader))
...
<script>
    $(function() {
        ...
        $(document).on("keyup",".check_phone_no",function() {
            ...
            if(number_length>11){
                alert("Maximum Number limit is ten digits");
                $(this).val('');
            }else{
        ...
        $(document).on("focusout", ".check_phone_no", function(e){
            ...
            {
                $.ajax({
                    data: { phone_no:phone_no, business_unit:business_unit },
        ...
        {{--//             url: "{{ route('check_if_phone_no_exist_hold_stage') }}",--}}
        {{--//             type: "post",--}}
        {{--//             beforeSend:function(){--}}
