<form id="frmTeamUser" action="{{ !isset($team_users) ? route('team.store') : route('team.update', $team_id) }}"
        ...
        <div class="row">
            <div class="col-md-4">
                    ...
                    <select name="lead_type" class="form-control" id="lead_type">
                        ...
                        @if (!empty($lead_type))
                            @foreach ($lead_type as $key => $type)
                                <option value="{{ $key }}" {{ ($key == $lead_type_name)? "selected" : "" }}>{{ $type }}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
                <div class="form-group">
                    ...
                    <select name="team_name" id="team_name" class="form-control required" style="width: 100%;"
                ...
                <div class="form-group">
                    ...
                    <select name="agent_type" id="agent_type" class="form-control required" style="width: 100%;"
                        required="required" aria-hidden="true">
                        <option value="0">Select</option>
                ...
                <div class="form-group">
                    ...
                    <select name="user_name" id="user_name" class="form-control select2" style="width: 100%;"
                        required="required" aria-hidden="true">
                        <option value="0">Select</option>
                        @if (!empty($user_arr))
                    ...
                    <select class="form-control" name="category" style="width: 100%;" aria-hidden="true">
                        ...
                        @if (!empty($project_cat))
                            @foreach ($project_cat as $key => $cat)
                                ...
                                @if ($key == $category)
                                    <option value="{{ $key }}" selected="selected">{{ $cat }}
                                    </option>
                                @else
                ...
                <div class="form-group">
                    <label>Area</label>
                    <select class="form-control select2" multiple="multiple" name="area[]" style="width: 100%;"
                        aria-hidden="true">
                        ...
                        @if (!empty($project_area))
                            ...
                            @foreach ($project_area as $key => $area)
                                @if (isset($selected[$key]))
                                    <option value="{{ $key }}" selected="selected">{{ $area }}
                                    </option>
                                @else
            ...
            <div class="col-md-8">
                ...
                <div class="box">
                    ...
                    <table id="user-table" width="50%"
                        class="table table-bordered table-striped table-hover data-table">
                        <thead>
                            <tr>
                                <th>Team User</th>
                                <th class="text-center" title="Head Of Department">Is HOD?</th>
                                <th class="text-center" title="Head Of Team">Is CHS?</th>
                                <th class="text-center" title="Team Leader">Is HOT?</th>
                                    ...
                                    <tr>
                                        <td>
                                            ...
                                            <input type="hidden" name="txtUserID[]"
                                                value="{{ $user->user_pk_no }}" />
                                            <input type="hidden" name="teammem_id[]"
                                                value="{{ $user->teammem_pk_no }}" />
                                        </td>
                                        <td align="center"><input type="checkbox"
                                                name="chkIsHod{{ $user->user_pk_no }}[]"
                                                {{ $user->hod_flag == 1 ? 'checked' : '' }} /></td>
                                        <td align="center"><input type="checkbox"
                                                name="chkIsHot{{ $user->user_pk_no }}[]"
                                                {{ $user->hot_flag == 1 ? 'checked' : '' }} /></td>
                                        <td align="center"><input type="checkbox"
                                                name="chkIsTL{{ $user->user_pk_no }}[]"
                                                {{ $user->team_lead_flag == 1 ? 'checked' : '' }} /></td>
                                        <td><input class="form-control" style="width: 50px;" type="text"
                                                name="teammem_seq{{ $user->user_pk_no }}"
                                                value="{{ $user->sl_no != '' ? ($user->sl_no > 0 ? $user->sl_no : '') : '' }}" />
                                        </td>
                                        <td class="text-center">
                                            <span class="btn bg-danger btn-xs remove-member"
                                                data-id="{{ $user->teammem_pk_no }}"><i
    ...
    <div class="modal-footer">
        ...
        <button type="submit"
            class="btn bg-purple btn-sm btnSaveUpdate">{{ empty($team_users) ? 'Save' : 'Update' }}</button>
        <span class="msg"></span>
    </div>
</form>
