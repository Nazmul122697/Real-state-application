...
<td class="text-center">
    <select id="cmbSalesAgent{{ $row->leadlifecycle_pk_no }}" class="form-control select2 select2-hidden-accessible" style="width: 100%;"
        ...
        @if(!empty($sales_agent) && isset($sales_agent[$row->project_category_pk_no][$row->project_area_pk_no]))
        @foreach($sales_agent[$row->project_category_pk_no][$row->project_area_pk_no] as $key => $agent)
